...
- Wake word filter, 30-second open-conversation window, boot-ready window, "repeat that" intent.
- Weather via NWS (free for commercial use, offline-aware, honest refusal when offline). Total offline mode ("go offline" blocks all internet features).
...
Fold 7 is listed as the primary target device, but active testing has continued to happen on the A32. Still needs its own dedicated validation session — voice, memory, face recognition, weather, wake word, all confirmed on Fold 7 specifically before shipping.
...
### 8. Open Source Credits — Priority 3, still needed
...
### 10. 16KB native library page-size alignment — ✓ RESOLVED
...
Scout already has a face, a voice, two brains (Gemini + TinyLlama), memory, weather, a wake word, reliable face recognition for the whole family, a complete onboarding flow, startup diagnostics, a working end-to-end model download flow, a settings screen, encrypted API key storage, and a stable icon. The A32 is stable through several root-caused fixes. 16KB alignment is confirmed resolved against a real signed release APK.

**Remaining launch blockers: Fold 7 dedicated stability testing, the Open Source Credits screen (in-app + website), and Play Store listing content (description, screenshots, content rating).** Everything else on the "must fix" list above is done.
