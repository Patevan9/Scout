# Patevan
Patevan — A private, local-first AI companion that lives on your device, not in the cloud.
Scout
A private, local-first AI companion for Android.
Scout is designed to be a helpful companion that runs primarily on your device, respecting privacy while providing voice interaction, memory, and intelligent assistance.
Features
Voice conversations
Local-first design
Privacy focused
On-device AI support
Memory and personalization
Weather and online services when available
No subscriptions
Philosophy
Scout is built around three principles:
Privacy first
Stability over complexity
User control
Current Status
Scout is currently in active development.
Planned features include:
Improved offline intelligence
Enhanced memory systems
Companion personality customization
Accessibility improvements
Future robotics integration
Requirements
Android device
Internet connection for online features
Optional AI API key for cloud-based intelligence
Developer
Created by Patrick Lippy through Lippy Robotics Lab.

## Development with OpenAI

Patevan was developed with assistance from OpenAI tools throughout the project.

### GPT-5
GPT-5 was used as a development assistant to:
- Brainstorm features and architecture
- Review Kotlin and Android code
- Help debug issues
- Improve documentation
- Refine user experience and interface ideas

### Codex
Codex was used to:
- Assist with implementing Kotlin code
- Suggest safe, incremental code changes
- Review existing code for bugs and stability
- Help troubleshoot Android-specific development issues

OpenAI tools acted as development assistants throughout the project. Final design decisions, testing, integration, and implementation were performed by the developer.

## Development Status

Patevan is currently in active development and is not yet feature complete. The project focuses on creating a privacy-first AI companion that performs as much processing locally on the device as possible while using cloud AI only when needed.

---

Copyright © 2026 Patrick Evan Lippy. All rights reserved.
