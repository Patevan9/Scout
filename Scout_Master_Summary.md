...
## August 16–19, 2026 — PRs #41–53 and #55 Merged: Memory-Integrity Backstops, Gaze Amplitude, Companion Moments Refinements, Family/Vision Routing, Self-Identity Guard, TeachExtractor Fix, Thinking-Face Lifecycle, Courtesy Extensions, Return-Greeting Priority, TTS Diagnostics, Welcome-Back Courtesy Routing, pendingAiAnswer Lifecycle Fix

✓ **Context.** A long, single continuous session covering fourteen merged PRs (#41–53, #55), almost all driven by real-device findings on the Fold 7 (a handful on the A32), each traced against actual source before any code changed, per this document's standing discipline. Two independent reviewers were used throughout — Claude implementing, ChatGPT independently re-reviewing several of the trickier diffs before merge, the TTS-diagnostics correlation fixes in particular, and later both PR #53 and PR #55 in full.
...
### PR #53 — merged: `CourtesyIntent.WELCOME_BACK`

✓ **PR #53 merged** (branch `claude/courtesy-welcome-back`, merge commit `ed5cf6c26d184a060d066a562f21064cfbf3f09a`) — closing a real-device gap found during PR #52's own investigation: Scout said its boot greeting, Patrick replied "Welcome back!", and it fell through `ScoutCourtesyMatcher`/`ScoutIntentRouter` entirely (no deterministic route existed for it), reaching Gemini/TinyLlama like an open question and entering Busy-Brain for a two-word acknowledgment — made easy to hit since the presence-reply window Scout's own boot/return greeting opens lets it through without the wake name. Matched via the existing exact-string table only (`"welcome back"`, `"welcome back {name}"`, `"glad you are back"`, `"good to have you back"`, `"nice to have you back"`) — deliberately deterministic and narrowly matched, **not** a loose "welcome back" substring/`contains()` route, with adversarial regression tests confirming "welcome back to the show"/"welcome back everyone to the meeting" stay unmatched. 11 new tests, 57/57 in the touched file passing locally, CI green.

### PR #55 — merged: `pendingAiAnswer` lifecycle fix (expiry, presence-completion guard, supersede)

✓ **PR #55 merged** (branch `claude/pending-answer-lifecycle-v2`, merge commit `0a6101ab0e6dc3b85f51e10218a2e35e1073af41`) — a second, independent bug traced from the same real-device report as PR #53: once `CourtesyIntent.WELCOME_BACK` exists, "Welcome back!" itself stops reaching the generative brain at all — but the report also surfaced a deeper structural gap that would still exist for *any* real, slow-to-resolve question. `pendingAiAnswer` (the Busy-Brain PR 2 holding slot for an answer that resolved while Scout was mid-utterance) had no expiry and no relevance check — it could drain onto **any** subsequent Scout utterance, including a fully unrelated proactive remark (a presence/return greeting, a Companion Moment), with a misleading "and about your earlier question" prefix, however much time had actually passed.

Implementation:
- New pure `ScoutPendingAnswerGate.decide(hasQueuedAnswer, wasPresenceInitiated, queuedAtMs, nowMs, maxAgeMs): Decision` (`NONE`/`DELIVER`/`HOLD`/`EXPIRED`) — **expiry is checked before hold**, so a presence-initiated completion can't indefinitely protect an answer that's actually gone stale. A presence-initiated TTS completion (boot greeting, idle-silence remark, return greeting, Companion Moment — confirmed the only 5 call sites using `isPresenceInitiated=true` in the file) cannot incorrectly drain a *fresh* queued answer — it's held for the next non-presence completion instead, still subject to a standalone `PENDING_AI_ANSWER_MAX_AGE_MS = 30_000L` constant (timer starts when the answer is actually queued, not when generation began — deliberately not derived from `PRESENCE_REPLY_WINDOW_MS`, a different concept).
- `onDone()`/`onError()` share one `handlePendingAnswerAfterTts()` call so the two callback paths can't drift into different rules.
- A genuinely new generative request (the shared `tryBegin()`-guarded dispatch point both Gemini and TinyLlama funnel through) supersedes and discards an older undelivered answer — never a courtesy/deterministic reply, which never calls `tryBegin()` at all.
...
- New `DiagLog.PendingAnswerDiscardReason { EXPIRED, SUPERSEDED, CONVERSATION_CLOSED }` + `logPendingAnswerDiscarded()` — controlled discard diagnostics, no speech text logged, same privacy convention as every other `DiagLog` method.
- The good Busy-Brain case (a real question → filler → the answer resolves during that same filler → drains right after it, prefixed) remains intact, verified by trace: the filler is spoken via `isStatusOnly`, never `isPresenceInitiated`, so its own completion is always non-presence → `DELIVER`.
- Verified this doesn't touch PR #52's TTS diagnostics (`AtomicInteger` dispatch counter, `ConcurrentHashMap` source map, the 5 `logTts*` events) — none of the non-deliver decision paths call `respond()`/`speak()`.

11 new tests in `ScoutPendingAnswerGateTest.kt`, all passing locally (kotlinc + JUnit, no Android SDK in this environment), alongside the 106 pre-existing tests in the closely-related pure-package subset re-confirmed unaffected. `MainActivity.kt`/`DiagLog.kt` verified by careful diff review — can't be compiled locally (Android-dependent types).

### Current development phase — real-device testing, not more coding

Both structural bugs from the "Welcome back!" real-device report are now merged (PR #53, PR #55). **The next step is real-device testing, not further implementation.** Specific tests before considering any more code changes:
1. Boot Scout and say "Welcome back!" — confirm Scout gives the short deterministic courtesy response instead of treating it as an AI question.
2. Ask a deliberately slower generative question and let the Busy-Brain filler speak — confirm the real completed answer still follows correctly afterward once the filler finishes (the "good Busy-Brain case" above).
3. Continue watching for the intermittent TTS problem PR #52 instrumented — a reply displays via captions but is sometimes never spoken, the mic stays off temporarily, Scout eventually recovers via the speaking watchdog. If it recurs, PR #52's `DiagLog` "TTS" events (`requested`/`speak_call`/`started`/`done`/`error`, correlated by dispatch id and `NORMAL`/`DRAINED_PENDING_ANSWER` source) should provide real evidence rather than requiring another guess.
4. Continue observing presence/return greetings and Companion Moments during normal real-device use, now that PR #55's presence-completion guard is live.

Do not start another fix as part of this pass — real-device observation comes first.

**All 14 PRs merged in this pass keep every prior PR's stated scope discipline** — no TruthDb schema changes, no `ScoutMemoryGate`/`ScoutIntentRouter`-widening beyond what's documented above, no touch to Companion Moments' core scoring/cooldowns, PR #38's calendar code, or `PeopleDb`.

**What's left:** real-device testing per the checklist above (PR #53's welcome-back routing, PR #55's pendingAiAnswer lifecycle, and continued TTS-lifecycle observation via PR #52's diagnostics) — not more coding; on-device verification for PR #42 (gaze amplitude — not yet re-tested), PR #44 (environment face-count re-check), and PR #45 (vision-gate wiring) specifically, none of which have a pure-Kotlin test surface; everything still outstanding from the August 12–15 entry below (PR #38's calendar-follow-up real-device validation, PR #37's disclosed paraphrase-layer gap) and the August 9–11 entry (Diana's S24 voice validation, the "Can you hear me?" STT evidence pull, a Working Memory design, the 30s/40s follow-up window's missing wake-word/vision check) — all unchanged by this pass.
