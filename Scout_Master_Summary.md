...
Status: Current. This pass catches up PRs #30–#35 (all merged) — the previous documentation sync, a Busy-Brain filler-timing polish, the fix for Patrick's name leaking into speaker-neutral conversation plus its one-time HabitLayer cleanup follow-up, the mic-chime mute fix, and the TTS default-voice selection fix. **PR #35's fix has not yet been installed or tested on Diana's Galaxy S24 — real-device validation there is still pending.**
